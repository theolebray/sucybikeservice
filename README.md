[index(3).html](https://github.com/user-attachments/files/26983313/index.3.html)[Uploading index(3<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="UTF-8">
  ...
  <script>
    ...
    function corps(prestation, prix, extra) {
      return `Bonjour,\n\nJe souhaite réserver la prestation suivante :\n\n✅ ${prestation} – ${prix}${extra ? '\n' + extra : ''}\n\nMon prénom et nom : \nMon numéro de téléphone : \n\nMerci !`;
    }

    function goMail(sujet, body) {
      const su = encodeURIComponent(sujet);
      const bo = encodeURIComponent(body);
      const to = encodeURIComponent(email);

      if (!isMobile()) {
        // PC → Gmail web, s'ouvre dans un nouvel onglet
        window.open(`https://mail.google.com/mail/?view=cm&fs=1&to=${to}&su=${su}&body=${bo}`, '_blank');
        return;
      }

      if (isIOS()) {
        // iPhone → tente l'app Gmail, sinon mailto
        const gmailScheme = `googlegmail://co?to=${to}&subject=${su}&body=${bo}`;
        const fallback = `mailto:${email}?subject=${su}&body=${bo}`;
        const t = Date.now();
        ...
        // Android → intent Gmail app
        const intent = `intent://compose?to=${to}&subject=${su}&body=${bo}#Intent;scheme=googlegmail;package=com.google.android.gm;end`;
        const fallback = `mailto:${email}?subject=${su}&body=${bo}`;
...
<p class="subtitle">👇 <strong>Appuyez sur une prestation</strong> pour envoyer une demande via Gmail</p>

<div class="grid">

  <!-- 1 Nettoyage Express -->
  <div class="card c-orange" onclick="goMail('Demande – Nettoyage Express (15€)', corps('Nettoyage Express', '15€', ''))">
    <div class="card-header">1 — Nettoyage Express <span class="tap-hint">✉️ Réserver</span></div>
    <div class="card-body">
  ...
  <!-- 2 Remise en route -->
  <div class="card c-navy" onclick="goMail('Demande – Remise en route (30€)', corps('Remise en route', '30€', ''))">
    <div class="card-header">2 — Remise en route <span class="tap-hint">✉️ Réserver</span></div>
    <div class="card-body">
  ...
  <!-- 3 Entretien complet -->
  <div class="card c-green" onclick="goMail('Demande – Entretien complet (50€)', corps('Entretien complet', '50€', ''))">
    <div class="card-header">3 — Entretien complet <span class="tap-hint">✉️ Réserver</span></div>
    <div class="card-body">
  ...
  <!-- 4 Entretien avancé -->
  <div class="card c-rust" onclick="goMail('Demande – Entretien avancé (70€)', corps('Entretien avancé', '70€', ''))">
    <div class="card-header">4 — Entretien avancé <span class="tap-hint">✉️ Réserver</span></div>
    <div class="card-body">
  ...
  <!-- 5 Lavage Pro -->
  <div class="card c-slate" onclick="goMail('Demande – Lavage Pro (60€)', corps('Lavage Pro', '60€', 'Option graissage chaîne +5€ : OUI / NON'))">
    <div class="card-header">5 — Lavage Pro <span class="tap-hint">✉️ Réserver</span></div>
    <div class="card-body">
  ...
  <!-- 6 Prestations à la carte -->
  <div class="card c-gray" style="cursor:default;">
    <div class="card-header">6 — Prestations à la carte <span class="tap-hint">✉️ Choisir</span></div>
    <div class="card-body">
      <div class="price">À partir de 15€</div>

      <div class="carte-item" onclick="event.stopPropagation(); goMail('Demande – Réglage freins (15€)', corps('Réglage freins', '15€', ''))">
        <span class="label">Réglage freins</span><span class="prix">15€</span>
      </div>
      <div class="carte-item" onclick="event.stopPropagation(); goMail('Demande – Chambre à air (20€/pièce)', corps('Chambre à air', '20€ / pièce', 'Nombre de pièces : '))">
        <span class="label">Chambre à air</span><span class="prix">20€ / pièce</span>
      </div>
      <div class="carte-item" onclick="event.stopPropagation(); goMail('Demande – Chaîne (30€/pièce)', corps('Chaîne', '30€ / pièce', 'Nombre de pièces : '))">
        <span class="label">Chaîne</span><span class="prix">30€ / pièce</span>
      </div>
      <div class="carte-item" onclick="event.stopPropagation(); goMail('Demande – Pneus complets (35€/pièce)', corps('Pneus complets', '35€ / pièce', 'Nombre de pièces : '))">
        <span class="label">Pneus complets</span><span class="prix">35€ / pièce</span>
      </div>
      <div class="carte-item" onclick="event.stopPropagation(); goMail('Demande – Réglage dérailleur (20€)', corps('Réglage dérailleur', '20€', ''))">
